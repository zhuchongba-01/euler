...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@zhongchongba/euler-evals:session" &&
				artifact.type !== "@zhongchongba/euler-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@zhongchongba/euler-evals:session" ? "sessions" : "sources";
