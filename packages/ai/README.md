...
```bash
npm install @zhongchongba/euler-ai
```

TypeBox exports are re-exported from `@zhongchongba/euler-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@zhongchongba/euler-ai';
import { builtinModels } from '@zhongchongba/euler-ai/providers/all';
...
import { openaiProvider } from '@zhongchongba/euler-ai/providers/openai';
import { openrouterProvider } from '@zhongchongba/euler-ai/providers/openrouter';
import { amazonBedrockProvider } from '@zhongchongba/euler-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@zhongchongba/euler-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@zhongchongba/euler-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@zhongchongba/euler-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@zhongchongba/euler-ai';
...
```typescript
import { validateToolCall, type Tool } from '@zhongchongba/euler-ai';
...
```typescript
import { builtinImagesModels } from '@zhongchongba/euler-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@zhongchongba/euler-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@zhongchongba/euler-ai';
import { openAICompletionsApi } from '@zhongchongba/euler-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@zhongchongba/euler-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@zhongchongba/euler-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@zhongchongba/euler-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@zhongchongba/euler-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@zhongchongba/euler-ai';
import { anthropicProvider } from '@zhongchongba/euler-ai/providers/anthropic';
import { openaiProvider } from '@zhongchongba/euler-ai/providers/openai';
import { googleProvider } from '@zhongchongba/euler-ai/providers/google';
...
- `@zhongchongba/euler-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@zhongchongba/euler-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@zhongchongba/euler-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@zhongchongba/euler-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@zhongchongba/euler-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@zhongchongba/euler-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@zhongchongba/euler-ai/bedrock-provider';
...
```bash
npx @zhongchongba/euler-ai login              # interactive provider selection
npx @zhongchongba/euler-ai login anthropic    # login to specific provider
npx @zhongchongba/euler-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@zhongchongba/euler-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@zhongchongba/euler-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@zhongchongba/euler-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@zhongchongba/euler-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@zhongchongba/euler-ai`.
