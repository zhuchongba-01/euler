# @zhongchongba/euler-telemetry

Vendor-neutral telemetry contracts and typed schema utilities for Euler packages.
...
Applications can use the in-memory reference or provide an adapter for OpenTelemetry, Sentry, logs, or another backend. Euler packages pass telemetry contexts explicitly and define their domain schemas separately.
...
```bash
npm install @zhongchongba/euler-telemetry
```
...
} from '@zhongchongba/euler-telemetry';
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@zhongchongba/euler-telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@zhongchongba/euler-telemetry';
...
`@zhongchongba/euler-telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@zhongchongba/euler-telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@zhongchongba/euler-agent-core';
...
- `@zhongchongba/euler-telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@zhongchongba/euler-ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@zhongchongba/euler-agent-core` owns and exports the Euler AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
...
The pi schemas use pi-owned `euler.ai.*`, `euler.harness.*`, and `euler.session.*` names. Adapters may translate them to backend conventions without changing the emitted pi vocabulary.
