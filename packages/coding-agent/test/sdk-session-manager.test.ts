...
describe("createAgentSession session manager defaults", () => {
	...
	it("uses agentDir for the default persisted session path", async () => {
		...
		expect(sessionFile?.replaceAll("\\", "/").startsWith(`${expectedSessionDir.replaceAll("\\", "/")}/`)).toBe(true);
	...
	it("exposes current session state to the built-in bash tool", async () => {
		...
		const result = await bashTool!.execute("test", {
			command: `printf '%s\\n' "$EULER_SESSION_ID" "$EULER_SESSION_FILE" "$EULER_PROVIDER" "$EULER_MODEL" "$EULER_REASONING_LEVEL"`,
		});
		const output = result.content
			.filter((item): item is { type: "text"; text: string } => item.type === "text")
			.map((item) => item.text)
