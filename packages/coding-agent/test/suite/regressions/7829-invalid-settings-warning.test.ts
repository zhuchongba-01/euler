...
describe("issue #7829 invalid settings warning", () => {
	...
	it("renders startup diagnostics inside the transcript", async () => {
		...
		const previousOffline = process.env.EULER_OFFLINE;
		...
		try {
			...
			if (previousOffline === undefined) delete process.env.EULER_OFFLINE;
			else process.env.EULER_OFFLINE = previousOffline;
			harness.cleanup();
		}
	});
});
