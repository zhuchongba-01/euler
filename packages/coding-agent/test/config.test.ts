...
function queryNpmGlobalRoot(argv: string[]): string {
	const result = spawnProcessSync(argv[0], argv.slice(1), {
		encoding: "utf-8",
		stdio: ["ignore", "pipe", "pipe"],
	});
	const stdout = result.stdout.trim();
	if (result.status !== 0 || !stdout) {
		throw new Error(`Failed to run ${argv.join(" ")}: exit ${result.status}`);
	}
	return stdout;
}
...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	const packageDir = join(root, "euler-agent");
	mkdirSync(packageDir, { recursive: true });
	tempDir = prefix;
	process.env.EULER_PACKAGE_DIR = packageDir;
...
function createFakePnpmScript(root: string): string {
	if (process.platform === "win32") {
		// cross-spawn quotes .cmd arguments, so compare with %~1 (unquoted).
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\euler-agent@0.67.68\\node_modules\\euler-agent\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("euler-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 euler-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getUpdateInstruction("euler-agent")).toBe(
			"Update euler-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("euler-agent");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "euler-agent"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 euler-agent`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("euler-agent", undefined, {
			packageName: "euler-agent",
			installSpec: "euler-agent@1.2.3",
		});

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "euler-agent@1.2.3"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 euler-agent@1.2.3`,
		});
	});
	...
	test("self-updates renamed packages from the current install prefix", () => {
		...
		const command = getSelfUpdateCommand("previous-agent", undefined, "@new-scope/euler");

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@new-scope/euler"],
			display: `npm --prefix ${prefix} uninstall -g previous-agent && npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @new-scope/euler`,
			steps: [
				{
					command: "npm",
					args: ["--prefix", prefix, "uninstall", "-g", "previous-agent"],
					display: `npm --prefix ${prefix} uninstall -g previous-agent`,
				},
				{
					command: "npm",
					args: [
					...
					display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @new-scope/euler`,
				},
			],
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		const { prefix } = createNpmPrefixInstall();
		// `npm root -g` with a configured prefix reports <prefix>\node_modules on
		// Windows and <prefix>\lib\node_modules elsewhere; place the fake install
		// where the real package manager reports it.
		const reportedRoot = queryNpmGlobalRoot(["npm", "--prefix", prefix, "root", "-g"]);
		const packageDir = join(reportedRoot, "euler-agent");
		mkdirSync(packageDir, { recursive: true });
		process.env.EULER_PACKAGE_DIR = packageDir;
		setExecPath(join(packageDir, "dist", "cli.js"));

		const command = getSelfUpdateCommand("euler-agent", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 euler-agent`,
		);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\euler-agent";
		process.env.EULER_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("euler-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 euler-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "euler-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 euler-agent",
		});
	});
	...
	test("self-updates renamed pnpm global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "pnpm",
			args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/euler"],
			display:
				"pnpm remove -g previous-agent && pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/euler",
			steps: [
				{
					command: "pnpm",
					args: ["remove", "-g", "previous-agent"],
					display: "pnpm remove -g previous-agent",
				},
				{
					command: "pnpm",
					args: ["install", "-g", "--ignore-scripts", "--config.minimumReleaseAge=0", "@new-scope/euler"],
					display: "pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @new-scope/euler",
				},
			],
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "euler-agent";
		const globalPackageDir = join(root, "11e9a", "node_modules", "euler-agent");
		...
		tempDir = temp;
		process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
		process.env.EULER_PACKAGE_DIR = storePackageDir;
	...
	test("self-updates renamed yarn global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "yarn",
			args: ["global", "add", "--ignore-scripts", "@new-scope/euler"],
			display: "yarn global remove previous-agent && yarn global add --ignore-scripts @new-scope/euler",
			steps: [
				{
					command: "yarn",
					args: ["global", "remove", "previous-agent"],
					display: "yarn global remove previous-agent",
				},
				{
					command: "yarn",
					args: ["global", "add", "--ignore-scripts", "@new-scope/euler"],
					display: "yarn global add --ignore-scripts @new-scope/euler",
				},
			],
		});
	});
	...
	test("self-updates renamed bun global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/euler"],
			display:
				"bun uninstall -g previous-agent && bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/euler",
			steps: [
				{
					command: "bun",
					args: ["uninstall", "-g", "previous-agent"],
					display: "bun uninstall -g previous-agent",
				},
				{
					command: "bun",
					args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@new-scope/euler"],
					display: "bun install -g --ignore-scripts --minimum-release-age=0 @new-scope/euler",
				},
			],
		});
	});
