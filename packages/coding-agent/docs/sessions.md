...
Euler saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.euler/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.
...
euler --session <path|id> # Use a specific session file or partial session ID
euler --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `euler -r` opens the same picker at startup.
...
When available, euler uses the `trash` CLI for deletion instead of permanently removing files.
...
```bash
euler --name "Refactor auth module"
euler --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `euler -r`.
...
When `/tree` switches away from one branch to another, euler can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
