...
```
~/.euler/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.euler/agent/sessions/`.

Euler also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, euler uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions in your project, inspect `node_modules/euler-agent/dist/` and `node_modules/@zhongchongba/euler-ai/dist/`.
...
The exported pi-ai `StopReason` type also includes `"pending"`, but that value is reserved for partial messages in streaming events. Terminal `done`/`error` messages replace it with a completion reason before euler persists the assistant message, so `"pending"` should never appear in session JSONL.
...
Use `customType` to identify your extension's entries on reload. Interactive mode can render custom entries via `euler.registerEntryRenderer(customType, renderer)`, but they still do not participate in LLM context.
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `euler.setSessionName()` in extensions.
