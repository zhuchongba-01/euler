...
Euler uses JSON settings files with project settings overriding global settings.

| Location | Scope |
|----------|-------|
| `~/.euler/agent/settings.json` | Global (all projects) |
| `.euler/settings.json` | Project (current directory) |
...
| `enableAnalytics` | boolean | `false` | Opt-in analytics data sharing. Currently only asked for during the experimental first-time setup (`EULER_EXPERIMENTAL=1`) |
...
For VS Code, include `--wait` so euler resumes after the editor exits:
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are explicitly needed. Setting it above `0` can make SDK/provider retries handle out-of-usage-limit errors before Euler sees them, which may block the agent until the provider quota resets in some circumstances.
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.euler/agent/npm/`; project-scoped npm packages install under `.euler/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.

### Tools

| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `defaultTools` | string[] | - | Built-in tools enabled initially. When omitted, Euler uses its standard defaults |
...
```json
{ "sessionDir": ".euler/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `EULER_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.euler/agent/settings.json` resolve relative to `~/.euler/agent`. Paths in `.euler/settings.json` resolve relative to `.pi`. Absolute paths and `~` are supported.
...
Project settings (`.euler/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.euler/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .euler/settings.json (project)
{
