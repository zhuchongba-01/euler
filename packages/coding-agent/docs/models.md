...
Add custom providers and models (Ollama, vLLM, LM Studio, proxies) via `~/.euler/agent/models.json`.
...
The `apiKey` value is a placeholder because Ollama ignores it. euler still treats models as requiring auth before they appear in `/model`, so keyless local servers should keep a dummy value, save a key for that provider with `/login`, or pass `--api-key` when selecting the model.

Some OpenAI-compatible servers do not understand the `developer` role used for reasoning-capable models. For those providers, set `compat.supportsDeveloperRole` to `false` so euler sends the system prompt as a `system` message instead. If the server also does not support `reasoning_effort`, set `compat.supportsReasoningEffort` to `false` too.
...
For `models.json`, shell commands are resolved at request time. euler intentionally does not apply built-in TTL, stale reuse, or recovery logic for arbitrary commands. Different commands need different caching and failure strategies, and euler cannot infer the right one.
...
| `thinkingLevelMap` | No | omitted | Maps euler thinking levels to provider values and marks unsupported levels (see below) |
...
`samplingParams` is a free-form object merged verbatim into every request body for the model, after the fields euler sets itself, so its keys win. Use it to send sampling parameters euler does not model — including server-specific ones like llama.cpp's `min_p` or vLLM's `top_k`:
...
Only OpenAI-compatible APIs apply it (`openai-completions`, `openai-responses`, `azure-openai-responses`); other APIs ignore it. Keys override euler's named request fields (for example a `temperature` key here beats the request-level temperature), so prefer it as the single source of sampling truth for a model. In `modelOverrides`, `samplingParams` merges per key with the base model's value.
...
Use `thinkingLevelMap` on a model to describe model-specific thinking controls. Keys are euler thinking levels: `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`. Maps may contain holes; for example, a model can expose `high` and `max` without exposing `xhigh`.
...
By default euler sends per-tool `eager_input_streaming: true`. If a proxy or Anthropic-compatible backend rejects that field, set `supportsEagerToolInputStreaming` to `false`. Euler will omit `tools[].eager_input_streaming` and send the legacy `fine-grained-tool-streaming-2025-05-14` beta header for tool-enabled requests instead.
...
| `supportsFinishReason` | Whether streamed responses include `finish_reason`. When `false`, euler infers `stop` or `toolUse` when the stream ends. Default: `true`. |
