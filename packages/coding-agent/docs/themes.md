> euler can create themes. Ask it to build one for your setup.
...
Euler loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.euler/agent/themes/*.json`
- Project: `.euler/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `euler.themes` entries in `package.json`
...
On first run, euler detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.euler/agent/themes
vim ~/.euler/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, euler reloads it automatically for immediate visual feedback.
...
Euler uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, euler falls back to the nearest approximation.
