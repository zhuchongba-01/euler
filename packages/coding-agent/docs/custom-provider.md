...
Extensions can register custom model providers via `euler.registerProvider()`. This enables:
...
export default function (euler: ExtensionAPI) {
  euler.registerProvider(createProvider({
    id: "native-local",
    name: "Native Local",
  ...
  euler.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. euler waits for the factory before startup continues, so the provider is available during interactive startup and to `euler --list-models`.
...
Use `euler.unregisterProvider(name)` to remove a provider that was previously registered via `euler.registerProvider(name, ...)`:
...
// Later, remove it
euler.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map euler levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
euler.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
Credentials are persisted in `~/.euler/agent/auth.json`:
...
- [anthropic-messages.ts](https://github.com/zhuchongba-01/euler/blob/main/packages/ai/src/api/anthropic-messages.ts) - Anthropic Messages API
- [mistral-conversations.ts](https://github.com/zhuchongba-01/euler/blob/main/packages/ai/src/api/mistral-conversations.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/zhuchongba-01/euler/blob/main/packages/ai/src/api/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/zhuchongba-01/euler/blob/main/packages/ai/src/api/openai-responses.ts) - OpenAI Responses API
- [google-generative-ai.ts](https://github.com/zhuchongba-01/euler/blob/main/packages/ai/src/api/google-generative-ai.ts) - Google Generative AI
- [bedrock-converse-stream.ts](https://github.com/zhuchongba-01/euler/blob/main/packages/ai/src/api/bedrock-converse-stream.ts) - AWS Bedrock
...
When a request exceeds the model's context window, euler can recover automatically by compacting the conversation and retrying. This recovery only kicks in if euler recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of euler's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/zhuchongba-01/euler/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message euler does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase euler recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
export default function (euler: ExtensionAPI) {
  ...
  euler.on("message_end", (event, ctx) => {
...
`message_end` runs before euler tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what euler checks. With this in place, euler will:
...
- Match a provider-specific pattern, not euler's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of euler's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/zhuchongba-01/euler/tree/main/packages/ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps euler thinking levels to provider/model-specific values; null marks a level unsupported. */
