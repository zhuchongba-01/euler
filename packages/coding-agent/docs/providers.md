...
Euler supports subscription-based providers via OAuth and API key providers via environment variables or auth file. Built-in catalogs ship with euler; configured providers may refresh newer catalogs and cache them in `~/.euler/agent/models-store.json` for offline use.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.euler/agent/auth.json` and auto-refresh when expired. OpenRouter instead mints a user-controlled API key that does not expire automatically.
...
Reference for environment variables and `auth.json` keys: [`const envMap`](https://github.com/zhuchongba-01/euler/blob/main/packages/ai/src/env-api-keys.ts) in [`packages/ai/src/env-api-keys.ts`](https://github.com/zhuchongba-01/euler/blob/main/packages/ai/src/env-api-keys.ts).

#### Auth File

Store credentials in `~/.euler/agent/auth.json`:
...
Use this when euler should use different provider settings than the project shell environment.
...
```bash
euler --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
euler --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
euler --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
For normal euler usage, prefer unified billing or stored BYOK. Inline BYOK requires configuring an additional upstream `Authorization` header for the Cloudflare AI Gateway provider, for example via a `models.json` provider/model override.
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
euler --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```

Euler automatically sets `x-session-affinity` for [prefix caching](https://developers.cloudflare.com/workers-ai/features/prompt-caching/) discounts.
...
Euler supports the llama.cpp router server. Configure it with `/login llama.cpp`, manage loaded models with `/llama`, and select a loaded model with `/model`.
