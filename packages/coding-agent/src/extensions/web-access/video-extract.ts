...
import { type ExtractedContent, type ExtractOptions, extractHeadingTitle, type FrameResult } from "./extract.ts";
...
async function uploadToFilesApi(
	...
	const initRes = await fetchGeminiApi(
		`${getUploadBase()}/files`,
		{
			method: "POST",
			headers: {
	...
	const uploadRes = await fetchGeminiApi(
		uploadUrl,
		{
			method: "PUT",
			headers: {
