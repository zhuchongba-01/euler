...
```ts
import { PiClient, type ByteTransportFactory } from "@zhongchongba/euler-client";
...
import { createUnixTransportFactory } from "@zhongchongba/euler-client/unix";
...
The `@zhongchongba/euler-client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `@zhongchongba/euler-client/unix` subpath.
