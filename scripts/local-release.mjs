...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `euler-${platform}.zip` : `euler-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createEulerShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "euler.cmd"))) {
			writeFileSync(join(installDirectory, "euler.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\euler.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "euler.ps1"), '& "$PSScriptRoot/node_modules/.bin/euler.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "euler.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\euler.exe" %*\r\n');
		writeFileSync(join(installDirectory, "euler.ps1"), '& "$PSScriptRoot/node_modules/.bin/euler.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "euler"), join(installDirectory, "euler"));
}
...
if (!options.skipInstall) {
	...
	createEulerShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createEulerShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "euler.cmd" : "euler")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "euler.cmd" : "euler")} --help`);
	}
}
