...
const internalPackagePrefix = "@zhongchongba/euler-";
const internalPackageNames = new Set(["euler-agent"]);
const installPackageName = "euler-agent-install";
...
function createInstallerPackageJson(codingAgentPackage) {
	const packageJson = {
		...
		private: true,
		description: "Lockfile root used by the Euler installer and updater.",
		dependencies: {
...
function validateGeneratedFiles(installerPackageJson, installLock, internalNames) {
	...
	for (const [lockPath, entry] of Object.entries(installLock.packages)) {
		...
		if (
			(packageName?.startsWith(internalPackagePrefix) || (packageName && internalPackageNames.has(packageName))) &&
