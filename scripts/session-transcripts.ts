 ...
 *   --analyze      Spawn Euler agents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("euler", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn Euler: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".euler/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn Euler agents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".euler/agent/AGENTS.md");
